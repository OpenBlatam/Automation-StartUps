#!/usr/bin/env python3
"""
Script de prueba final para el Sistema de Control de Inventario v7.0
Incluye todas las funcionalidades: ML, optimización, tiempo real, integración, monitoreo, IA avanzada, IoT, AR, análisis predictivo, logística
"""
...
def test_predictive_functionality():
    """Prueba la funcionalidad de análisis predictivo"""
    print_section("Funcionalidad de Análisis Predictivo")
    
    try:
        from services.predictive_analytics_service import advanced_predictive_analytics_service
        
        # Probar preparación de datos
        print("🔍 Probando preparación de datos de series temporales...")
        data = advanced_predictive_analytics_service.prepare_time_series_data(days_back=30)
        ...
        if patterns.get('success'):
            print(f"✅ Patrones analizados: {patterns['total_products']} productos")
        else:
            print(f"⚠️  Análisis de patrones: {patterns.get('error', 'Error desconocido')}")
        
        return True
    except Exception as e:
        print(f"❌ Error probando funcionalidad de análisis predictivo: {str(e)}")
        traceback.print_exc()
        return False

def test_logistics_functionality():
    """Prueba la funcionalidad de logística"""
    print_section("Funcionalidad de Logística")
    
    try:
        from services.logistics_service import logistics_optimization_service
        
        # Probar estado de vehículos
        print("🔍 Probando estado de vehículos...")
        vehicle_status = logistics_optimization_service.get_vehicle_status()
        
        if vehicle_status['success']:
            print(f"✅ Estado de vehículos: {vehicle_status['total_vehicles']} vehículos")
            print(f"✅ Vehículos disponibles: {vehicle_status['available_vehicles']}")
        else:
            print(f"⚠️  Estado de vehículos: {vehicle_status['error']}")
        
        # Probar dashboard de logística
        print("🔍 Probando dashboard de logística...")
        dashboard = logistics_optimization_service.get_logistics_dashboard()
        
        if dashboard['success']:
            print(f"✅ Dashboard de logística: {dashboard['dashboard']['vehicles']['total']} vehículos")
        else:
            print(f"⚠️  Dashboard de logística: {dashboard['error']}")
        
        return True
    except Exception as e:
        print(f"❌ Error probando funcionalidad de logística: {str(e)}")
        traceback.print_exc()
        return False
...
def main():
    ...
    print_header("Sistema de Control de Inventario v7.0 - Pruebas Finales")
    ...
    tests = [
        ...
        ("Rutas", test_routes),
        ("Funcionalidad de Análisis Predictivo", test_predictive_functionality),
        ("Funcionalidad de Logística", test_logistics_functionality),
    ...
    print("\n" + "="*90)
    print("🚀 Sistema de Control de Inventario v7.0")
    ...
    print("   • API RESTful completa (150+ endpoints)")
