# ENTREGABLE 2: ECUACIONES DIFERENCIALES
...
**Reporte de Investigación: Aplicaciones de las Ecuaciones Diferenciales**
...
## APLICACIONES A LA INGENIERÍA EN GENERAL Y A [TU CARRERA] EN PARTICULAR

### Aplicaciones Generales en Ingeniería
...
### Aplicaciones Específicas en [TU CARRERA]

*[Nota: Necesito saber cuál es tu carrera específica para personalizar esta sección. Las aplicaciones variarán significativamente entre Ingeniería Mecánica, Eléctrica, Química, Civil, Industrial, etc.]*

#### Ejemplos de Aplicaciones por Carrera:
...
## PARTE B: EJERCICIOS DE REPASO

### EJERCICIOS DE MÉTODOS ANTERIORES
...
$$x^2(1 + v^2)dx - 2x^2v(v dx + x dv) = 0$$
...
**Solución:**
Esta es una ecuación lineal de primer orden de la forma $y' + P(x)y = Q(x)$ donde $P(x) = \frac{1}{x}$ y $Q(x) = x^2$.

El factor integrante es:
$$\mu(x) = e^{\int P(x)dx} = e^{\int \frac{1}{x}dx} = e^{\ln|x|} = |x| = x \text{ (para } x > 0)$$

Multiplicando la ecuación por el factor integrante:
$$xy' + y = x^3$$

El lado izquierdo es $\frac{d}{dx}(xy)$, entonces:
$$\frac{d}{dx}(xy) = x^3$$
...
La solución general es:
$$y = y_h + y_p = C_1 e^x + C_2 e^{2x} + 2e^{3x}$$

#### Ejercicio 7: Solución de EDO usando Transformada de Laplace
**Resolver:** $y'' + 4y' + 4y = e^{-2t}$ con $y(0) = 1$ e $y'(0) = 0$

**Solución:**
Aplicando la transformada de Laplace a ambos lados:
$$\mathcal{L}\{y''\} + 4\mathcal{L}\{y'\} + 4\mathcal{L}\{y\} = \mathcal{L}\{e^{-2t}\}$$

$$s^2Y(s) - sy(0) - y'(0) + 4(sY(s) - y(0)) + 4Y(s) = \frac{1}{s+2}$$

$$s^2Y(s) - s - 0 + 4sY(s) - 4 + 4Y(s) = \frac{1}{s+2}$$

$$Y(s)(s^2 + 4s + 4) - s - 4 = \frac{1}{s+2}$$

$$Y(s)(s+2)^2 = \frac{1}{s+2} + s + 4 = \frac{1 + s(s+2) + 4(s+2)}{s+2}$$

$$Y(s)(s+2)^2 = \frac{1 + s^2 + 2s + 4s + 8}{s+2} = \frac{s^2 + 6s + 9}{s+2}$$

$$Y(s) = \frac{s^2 + 6s + 9}{(s+2)^3} = \frac{(s+3)^2}{(s+2)^3}$$

Usando fracciones parciales:
$$\frac{(s+3)^2}{(s+2)^3} = \frac{A}{s+2} + \frac{B}{(s+2)^2} + \frac{C}{(s+2)^3}$$

Resolviendo: $A = 1$, $B = 2$, $C = 1$

Por tanto:
$$Y(s) = \frac{1}{s+2} + \frac{2}{(s+2)^2} + \frac{1}{(s+2)^3}$$

Aplicando la transformada inversa:
$$y(t) = e^{-2t} + 2te^{-2t} + \frac{t^2}{2}e^{-2t} = e^{-2t}\left(1 + 2t + \frac{t^2}{2}\right)$$

### EJERCICIOS ADICIONALES DE ESTA SEMANA

#### Ejercicio 8: Serie de Fourier
**Desarrollar en serie de Fourier la función periódica de período $2\pi$ dada por:**
...
$$a_0 = \frac{1}{\pi} \int_{-\pi}^{\pi} f(x) dx = \frac{1}{\pi} \int_{0}^{\pi} x dx = \frac{1}{\pi} \left[\frac{x^2}{2}\right]_0^{\pi} = \frac{\pi}{2}$$

$$a_n = \frac{1}{\pi} \int_{-\pi}^{\pi} f(x) \cos(nx) dx = \frac{1}{\pi} \int_{0}^{\pi} x \cos(nx) dx$$

$$= \frac{1}{\pi} \left[\frac{x \sin(nx)}{n} + \frac{\cos(nx)}{n^2}\right]_0^{\pi} = \frac{1}{\pi} \left[\frac{\cos(n\pi) - 1}{n^2}\right]$$

$$= \frac{(-1)^n - 1}{\pi n^2} = \begin{cases}
0 & \text{si } n \text{ es par} \\
-\frac{2}{\pi n^2} & \text{si } n \text{ es impar}
\end{cases}$$

$$b_n = \frac{1}{\pi} \int_{-\pi}^{\pi} f(x) \sin(nx) dx = \frac{1}{\pi} \int_{0}^{\pi} x \sin(nx) dx$$

$$= \frac{1}{\pi} \left[-\frac{x \cos(nx)}{n} + \frac{\sin(nx)}{n^2}\right]_0^{\pi} = \frac{1}{\pi} \left[-\frac{\pi \cos(n\pi)}{n}\right]$$
...
$$= \frac{\pi}{4} + \sum_{k=1}^{\infty} \left[ -\frac{2}{\pi (2k-1)^2} \cos((2k-1)x) + \frac{(-1)^{2k}}{2k-1} \sin((2k-1)x) \right]$$

#### Ejercicio 9: Ecuación Diferencial con Condiciones Iniciales
**Resolver:** $x''(t) + 4x(t) = f(t)$ donde $x(1) = 0$ e $x'(1) = n$
...
La ecuación característica es: $r^2 + 4 = 0$, entonces $r = \pm 2i$
...
$$x(t) = C_1 \cos(2t) + C_2 \sin(2t) + x_p(t)$$

Aplicando las condiciones iniciales:
$$x(1) = C_1 \cos(2) + C_2 \sin(2) + x_p(1) = 0$$
$$x'(1) = -2C_1 \sin(2) + 2C_2 \cos(2) + x_p'(1) = n$$

Resolviendo este sistema de ecuaciones obtenemos los valores de $C_1$ y $C_2$.

#### Ejercicio 10: Ecuación Diferencial Parcial
**Verificar que la función dada es solución de la ecuación diferencial parcial:**

$$\frac{\partial^2 u}{\partial x \partial y} = x + y^3$$

**Función propuesta:**
$$u(x,y) = \frac{1}{2}x^2 y + \frac{1}{4}x y^4 - \frac{1}{4}y^4 + 2y^2 - \frac{9}{2}y + x^2 - 3x - 29$$

**Verificación:**

Calculamos las derivadas parciales:

$$\frac{\partial u}{\partial x} = \frac{1}{2} \cdot 2x \cdot y + \frac{1}{4} y^4 + 2x - 3 = xy + \frac{1}{4}y^4 + 2x - 3$$

$$\frac{\partial^2 u}{\partial x \partial y} = \frac{\partial}{\partial y}\left(\frac{\partial u}{\partial x}\right) = \frac{\partial}{\partial y}\left(xy + \frac{1}{4}y^4 + 2x - 3\right)$$

$$= x + \frac{1}{4} \cdot 4y^3 = x + y^3$$

Como $\frac{\partial^2 u}{\partial x \partial y} = x + y^3$, se verifica que la función propuesta es efectivamente una solución de la ecuación diferencial parcial dada.

---

**Nota:** Para completar los ejercicios 2-7, sería necesario tener los enunciados completos y claros, ya que algunos contienen notación confusa o están incompletos en el texto original proporcionado.
