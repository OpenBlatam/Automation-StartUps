#!/usr/bin/env python3
"""
Script de prueba final para el Sistema de Control de Inventario v5.0
Incluye todas las funcionalidades: ML, optimización, tiempo real, integración, monitoreo, IA avanzada, blockchain
"""
...
def test_ai_blockchain_services():
    """Prueba los servicios de IA avanzada y blockchain"""
    print_section("Servicios de IA Avanzada y Blockchain")
    ...
    try:
        ...
        print("✅ Servicio de blockchain importado correctamente")
    except Exception as e:
        print(f"❌ Error importando servicio de blockchain: {str(e)}")
        return False
...
def test_routes():
    ...
    try:
        ...
        print(f"❌ Error importando blueprint de IA & blockchain: {e}")
        return False
...
def test_ai_functionality():
    """Prueba la funcionalidad de IA avanzada"""
    print_section("Funcionalidad de IA Avanzada")
    ...
    try:
        ...
        insights = advanced_ai_service.generate_ai_insights()
        
        if insights:
            print(f"✅ Insights generados: {len(insights)} insights")
        else:
            print("⚠️  No se generaron insights")
        
        # Probar rendimiento de modelos
        print("🔍 Probando rendimiento de modelos...")
        performance = advanced_ai_service.get_model_performance()
        
        if 'error' not in performance:
            print(f"✅ Rendimiento de modelos obtenido: {performance.get('total_models', 0)} modelos")
        else:
            print(f"⚠️  Rendimiento de modelos: {performance['error']}")
        
        return True
    except Exception as e:
        print(f"❌ Error probando funcionalidad de IA: {str(e)}")
        traceback.print_exc()
        return False

def test_blockchain_functionality():
    """Prueba la funcionalidad de blockchain"""
    print_section("Funcionalidad de Blockchain")
    
    try:
        from services.blockchain_service import blockchain_service
        
        # Probar información de la cadena
        print("🔍 Probando información de blockchain...")
        info = blockchain_service.get_chain_info()
        
        if info:
            print(f"✅ Información de blockchain: {info.get('chain_length', 0)} bloques")
            print(f"✅ Total transacciones: {info.get('total_transactions', 0)}")
            print(f"✅ Cadena válida: {info.get('chain_valid', False)}")
        else:
            print("⚠️  No se pudo obtener información de blockchain")
        
        # Probar verificación de cadena
        print("🔍 Probando verificación de cadena...")
        is_valid = blockchain_service.verify_chain()
        print(f"✅ Verificación de cadena: {'Válida' if is_valid else 'Inválida'}")
        
        return True
    except Exception as e:
        print(f"❌ Error probando funcionalidad de blockchain: {str(e)}")
        traceback.print_exc()
        return False
...
def main():
    ...
    print_header("Sistema de Control de Inventario v5.0 - Pruebas Finales")
    ...
    tests = [
        ...
        ("Servicios de IA Avanzada y Blockchain", test_ai_blockchain_services),
        ("Rutas", test_routes),
        ("Funcionalidad de IA Avanzada", test_ai_functionality),
        ("Funcionalidad de Blockchain", test_blockchain_functionality),
    ...
    print("\n" + "="*90)
    print("🚀 Sistema de Control de Inventario v5.0")
    ...
    print("   • API RESTful completa (100+ endpoints)")
    ...
    print("   • Sistema de Blockchain para trazabilidad")
    print("   • Análisis combinado IA + Blockchain")
    print("="*90)
