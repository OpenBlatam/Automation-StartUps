...
def _send_slack(text: str) -> None:
	try:
		if _SLACK_PROVIDER:
			conn_id = Variable.get("etl_improved__slack_conn_id", os.environ.get("SLACK_CONN_ID", "slack_default"))
			SlackWebhookHook(slack_webhook_conn_id=conn_id, message=text).execute()
			return
		# fallback: raw webhook env var
...
def etl_improved() -> None:
	...
	def _on_failure(context: Dict[str, Any]) -> None:
		...
		_send_slack(f":rotating_light: etl_improved task failed: {task_id}")
