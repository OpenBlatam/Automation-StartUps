...
def create_master_pdf():
    ...
    doc = SimpleDocTemplate(
        "bioclones_novela_master.pdf", 
        pagesize=A4,
        rightMargin=2*cm, 
        leftMargin=2*cm,
        topMargin=3*cm, 
        bottomMargin=2.5*cm,
        title="Bioclones - Edición Maestra Digital",
        author="Sistema de Transcripción y Formateo Digital Avanzado",
        subject="Ciencia Ficción - Clonación - Identidad Humana - Edición Maestra",
        creator="Sistema de Generación Automática de Documentos",
        keywords="ciencia ficción, clonación, identidad, tecnología, futuro, manuscrito, edición maestra, digital"
    )
    ...
    # Título principal con decoración maestra
    story.append(Paragraph("◆ BIOCLONES ◆", title_style))
    story.append(Spacer(1, 1*inch))
    story.append(Paragraph("Edición Maestra Digital", subtitle_style))
    story.append(Spacer(1, 2.5*inch))
    
    # Línea decorativa maestra
    story.append(Paragraph("◆" * 60, body_style))
    story.append(Spacer(1, 2*inch))
    
    # Información del manuscrito maestra
    info_text = """
    <para align="center" fontSize="20" fontName="Helvetica" textColor="#374151">
    ...
    Formato: Edición Maestra Digital | Versión: 1.0
    </font>
    </para>
    """
    ...
    # Índice maestra
    story.append(Paragraph("◆ ÍNDICE MAESTRO ◆", chapter_style))
    story.append(Spacer(1, 70))
    
    # Tabla de índice maestra
    index_data = [
        ['Capítulo', 'Título', 'Página', 'Temas'],
        ['I', 'Nuestro objetivo', '3', 'Introducción, Complejo'],
        ['II', 'Elisa y Canon', '4', 'Transición'], 
        ['III', 'Error y Acierto', '5', 'Filosofía'],
        ['IV', 'Dios Clones', '6', 'Ciencia, Ética'],
        ['V', 'Sophie y yo', '8', 'Relaciones'],
        ['VI', 'Borradores', '9', 'Poesía, Reflexión']
    ]
    
    index_table = Table(index_data, colWidths=[2*cm, 8*cm, 2*cm, 4*cm])
    ...
    # Nota sobre el manuscrito maestra
    nota_text = """
    ...
    Edición maestra digital para preservación, estudio y análisis.</i>
    </para>
    """
    ...
    # Fin del manuscrito maestra
    story.append(Paragraph("◆ FIN DEL MANUSCRITO ◆", section_style))
    story.append(Spacer(1, 70))
    
    # Información de cierre maestra
    cierre_text = """
    <para align="center" fontSize="18" fontName="Helvetica" textColor="#6b7280">
    ...
    Formato: Edición Maestra Digital | Fecha: """ + datetime.now().strftime("%B %Y") + """<br/>
    <br/>
    <i>Edición especial de preservación digital y análisis académico</i>
    </font>
    </para>
    """
    story.append(Paragraph(cierre_text, body_style))
    
    # Función para numerar páginas maestra
    def add_page_number(canvas, doc):
        ...
        # Agregar línea decorativa maestra
        canvas.setStrokeColor(HexColor('#f59e0b'))
        canvas.setLineWidth(4)
        canvas.line(2*cm, 1.5*cm, 19*cm, 1.5*cm)
        # Agregar símbolo decorativo maestra
        canvas.setFillColor(HexColor('#f59e0b'))
        canvas.circle(10.5*cm, 1.5*cm, 5, fill=1)
        canvas.setFillColor(HexColor('#ffffff'))
        canvas.circle(10.5*cm, 1.5*cm, 3, fill=1)
        canvas.setFillColor(HexColor('#f59e0b'))
        canvas.circle(10.5*cm, 1.5*cm, 1, fill=1)
        canvas.restoreState()
    
    # Construir el PDF
    doc.build(story, onFirstPage=add_page_number, onLaterPages=add_page_number)
    print("PDF maestra creado exitosamente: bioclones_novela_master.pdf")
