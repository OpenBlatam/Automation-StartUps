...
def create_enhanced_pdf():
    ...
    story.append(Paragraph("Una Novela", ParagraphStyle('Subtitle', parent=styles['Heading2'], fontSize=18, alignment=TA_CENTER, textColor=HexColor('#2d3748'))))
    story.append(Spacer(1, 2*inch))
    story.append(Paragraph("─" * 50, body_style))
    story.append(Spacer(1, 1*inch))
    story.append(Paragraph(f"Manuscrito original transcrito digitalmente<br/>{datetime.now().strftime('%B %Y')}", body_style))
    ...
    index_table.setStyle(TableStyle([
        ...
        ('LINEBELOW', (0, 0), (-1, 0), 1, HexColor('#dc2626')),
        ('PADDING', (0, 0), (-1, -1), 10),
    ]))
    ...
    story.append(Paragraph("¡Bienvenida a la Capital Biológica! – exclamó con gran calidez", dialogue_style))
    ...
    story.append(Paragraph("Capítulo", ParagraphStyle('Section', parent=styles['Heading3'], fontSize=18, textColor=HexColor('#1a365d'))))
    ...
    story.append(Paragraph("Rápido", ParagraphStyle('Section', parent=styles['Heading3'], fontSize=18, textColor=HexColor('#1a365d'))))
    ...
    """
    story.append(Paragraph(senti_text, ParagraphStyle('Poetry', parent=styles['Normal'], fontSize=15, alignment=TA_CENTER, fontName='Times-Italic', textColor=HexColor('#4a5568'), leftIndent=60, rightIndent=60)))
    story.append(Spacer(1, 25))
    ...
    story.append(Paragraph("Fin del manuscrito", ParagraphStyle('End', parent=styles['Heading3'], fontSize=18, alignment=TA_CENTER, textColor=HexColor('#dc2626'))))
    ...
    print("PDF enhanced creado exitosamente: bioclones_novela_enhanced.pdf")
