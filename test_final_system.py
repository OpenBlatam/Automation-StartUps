#!/usr/bin/env python3
"""
Script de prueba final para el Sistema de Control de Inventario v3.0
Incluye todas las funcionalidades avanzadas: ML, optimización, tiempo real
"""
...
def test_ml_functionality():
    """Prueba la funcionalidad de machine learning"""
    print_section("Funcionalidad de Machine Learning")
    
    try:
        from services.advanced_ml_service import advanced_ml_service
        
        # Probar preparación de datos
        print("🔍 Probando preparación de datos...")
        df = advanced_ml_service.prepare_training_data(days_back=30)
        ...
        print("🔍 Probando entrenamiento de modelos...")
        result = advanced_ml_service.train_models()
        
        if 'error' in result:
            print(f"⚠️  Entrenamiento de modelos: {result['error']}")
        else:
            print(f"✅ Modelos entrenados: {result['models_trained']}")
            print(f"✅ Mejor modelo: {result['best_model']}")
            print(f"✅ Mejor R²: {result['best_r2']:.3f}")
        
        return True
    except Exception as e:
        print(f"❌ Error probando funcionalidad de ML: {str(e)}")
        traceback.print_exc()
        return False

def test_optimization_functionality():
    """Prueba la funcionalidad de optimización"""
    print_section("Funcionalidad de Optimización")
    ...
    try:
        ...
        result = inventory_optimization_service.optimize_inventory(
            products=[1, 2, 3],  # IDs de productos de prueba
            budget_constraint=50000,
            warehouse_capacity=500
        )
        
        if not result.solution:
            print("⚠️  No se pudo encontrar una solución óptima")
        else:
            print(f"✅ Optimización completada")
            print(f"✅ Costo total: ${result.total_cost:.2f}")
            print(f"✅ Valor total: ${result.total_value:.2f}")
            print(f"✅ Nivel de servicio: {result.service_level:.1%}")
            print(f"✅ Iteraciones: {result.iterations}")
            print(f"✅ Tiempo de convergencia: {result.convergence_time:.2f}s")
        
        return True
    except Exception as e:
        print(f"❌ Error probando funcionalidad de optimización: {str(e)}")
        traceback.print_exc()
        return False

def test_realtime_functionality():
    """Prueba la funcionalidad de tiempo real"""
    print_section("Funcionalidad de Tiempo Real")
    ...
    try:
        ...
        print(f"❌ Error probando funcionalidad de tiempo real: {str(e)}")
        traceback.print_exc()
        return False
...
def main():
    ...
    print_header("Sistema de Control de Inventario v3.0 - Pruebas Finales")
    ...
    tests = [
        ...
        ("Rutas", test_routes),
        ("Funcionalidad de Machine Learning", test_ml_functionality),
        ("Funcionalidad de Optimización", test_optimization_functionality),
        ("Funcionalidad de Tiempo Real", test_realtime_functionality),
    ...
    print("\n" + "="*70)
    print("🚀 Sistema de Control de Inventario v3.0")
    print("🧠 Funcionalidades de Machine Learning:")
    print("   • Predicción de demanda con múltiples algoritmos")
    print("   • Análisis ABC automático")
    print("   • Clustering de productos")
